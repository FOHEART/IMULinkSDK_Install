...
/**
 * @brief Test adding suit serial numbers to the whitelist and verify behavior.
 * @brief-cn 测试将套装序列号添加到白名单并验证其行为。
 *
 * Example:
 * std::vector<unsigned int> whitelist = { 0x1403403E };
 * IMULinkSDK_AddToWhitelist(whitelist);
 *
 * Adding serial 0x1403403E to the whitelist causes the SDK to allow only that suit's
 * data; calls to IMULinkSDK_GetSuitFrames will not return frames for other suits.
...
static int IMULinkSDK_openCloseLoopTest(int testRound)
{
	...
	for (int i = 0; i < testRound; ++i)
	{
		...
		{
			/** @brief Get the current whitelist and print to console. / 获取当前白名单列表并打印到控制台。 */
			std::vector<unsigned int>& currentWhitelist = IMULinkSDK_GetWhitelist();
			std::cout << "Current whitelist: ";
			for (const auto& sn : currentWhitelist)
			{
