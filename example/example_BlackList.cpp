...
/**
 * @brief Test adding suit serial numbers to the blacklist and verify they are ignored.
 * @brief-cn 测试将套装序列号添加到黑名单并验证它们被正确忽略的功能。
 *
 * Example:
 * std::vector<unsigned int> blacklist = { 0x1403403E };
 * IMULinkSDK_AddToBlacklist(blacklist);
 *
 * Adding serial 0x1403403E to the blacklist causes the SDK to ignore that suit;
 * calls to IMULinkSDK_GetSuitFrames will not return frames for the blacklisted suit.
...
static int IMULinkSDK_openCloseLoopTest(int testRound)
{
	...
	for (int i = 0; i < testRound; ++i)
	{
		...
		/** @brief Add example suit serial to the blacklist (e.g., 0x1403403E).
		 *  @brief-cn 将示例套装序列号添加到黑名单（例如 0x1403403E）。
		 */
		std::vector<unsigned int> blacklist = { 0x1403403E };
		IMULinkSDK_AddToBlacklist(blacklist);

		{
			/** @brief Get the current blacklist and print to console.
			 *  @brief-cn 获取当前黑名单列表并打印到控制台。
			 */
		...
		std::thread pollThread([&]() {
			while (!stopPolling.load())
			{
				...
				/** @brief Stop polling when exceeding maxRunTime. / 超过最大运行时间时停止轮询。 */
				...
				if (!frames.empty())
				{
					...
					{
						/** @brief Index within the suit frames to print (default pelvis). / 要打印的索引（默认 骨盆）。 */
						KHS53_SkeletonIndex printIndex = KHS53_Pelvis;
...
int main(int argc, char* argv[])
{
	...
	/** @brief Wait for user input before exiting. / 等待用户输入以退出。 */
	std::cin.get();
	return 0;
}
